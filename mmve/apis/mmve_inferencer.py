...
from .inferencers.base_mmve_inferencer import InputsType

class MmveInferencer:
    """MmveInferencer API for mmve models inference.
    ...
    def __init__(self,
                 ...
                 seed: int = 2022,
                 **kwargs) -> None:
        init_default_scope('mmve')
        MmveInferencer.init_inference_supported_models_cfg()
        inferencer_kwargs = {}
    ...
    def init_inference_supported_models_cfg() -> None:
        if not MmveInferencer.inference_supported_models_cfg_inited:
            if osp.exists(
            ...
            for model_name in MmveInferencer.inference_supported_models:
                ...
                MmveInferencer.inference_supported_models_cfg[
                    model_name] = {}
                MmveInferencer.inference_supported_models_cfg[model_name][
                    'task'] = task  # noqa
                MmveInferencer.inference_supported_models_cfg[model_name][
                    'settings'] = parsed_yaml['Models']  # noqa

            MmveInferencer.inference_supported_models_cfg_inited = True

    @staticmethod
    def get_inference_supported_models() -> List:
        """static function for getting inference supported modes."""
        return MmveInferencer.inference_supported_models
    ...
    def get_inference_supported_tasks() -> List:
        ...
        supported_task = set()
        for key in MmveInferencer.inference_supported_models_cfg.keys():
            if MmveInferencer.inference_supported_models_cfg[key]['task'] \
               not in supported_task:
                supported_task.add(MmveInferencer.
    ...
    def get_task_supported_models(task: str) -> List:
        ...
        supported_models = []
        for key in MmveInferencer.inference_supported_models_cfg.keys():
            if MmveInferencer.inference_supported_models_cfg[key][
                    'task'] == task:
